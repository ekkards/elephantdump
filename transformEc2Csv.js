...
function transformIpPermissions(accountID, securitygroups, property, defaultaccess) {
    ...
    for (i = 0; i < securitygroups.length; i++) {
        ...
        for (var j = 0; j < ip.length; j++) {
            ...
            if (newIp.ToPort && newIp.FromPort == newIp.ToPort)
        ...
        if (ip.length === 0)
            ipperms.push({
                GroupId: sgid,
                Region: securitygroups[i].Region,
                FromPort: defaultaccess,
                IpProtocol: defaultaccess,
                Source: defaultaccess
            });        // empty line = no access
    }
